...
from app.routers import auth_db as auth, analysis, screening, queue, sse, health, favorites, config, reports, database, operation_logs, tags, tushare_init, akshare_init, baostock_init, historical_data, multi_period_sync, financial_data, news_data, usage_statistics, model_capabilities, cache, logs
...
async def _print_config_summary(logger):
    ...
    try:
        logger.info("=" * 70)
        logger.info("📋 QuantAgent-Invest Configuration Summary")
        logger.info("=" * 70)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        await close_db()
        logger.info("QuantAgent-Invest FastAPI backend stopped")


# 创建FastAPI应用
app = FastAPI(
    title="QuantAgent-Invest API",
