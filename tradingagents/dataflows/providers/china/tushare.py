...
class TushareProvider(BaseStockDataProvider):
    ...
    def connect_sync(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"[ERROR] Tushare连接失败: {e}")
            return False

    async def connect(self) -> bool:
        """异步连接到Tushare"""
        if not TUSHARE_AVAILABLE:
            self.logger.error("[ERROR] Tushare库不可用")
            return False
        ...
        try:
            ...
            if db_token:
                try:
                    self.logger.info(f"[INFO] 尝试使用数据库中的 Tushare Token (超时: {test_timeout}秒)...")
                    ts.set_token(db_token)
                    self.api = ts.pro_api()
            ...
            if env_token:
                try:
                    self.logger.info(f"[INFO] 尝试使用 .env 中的 Tushare Token (超时: {test_timeout}秒)...")
                    ts.set_token(env_token)
                    self.api = ts.pro_api()
                    ...
                    try:
                        ...
                        self.logger.error(f"[ERROR] .env Token 测试超时 ({test_timeout}秒)")
                        return False
                    ...
                    if test_data is not None and not test_data.empty:
                        ...
                        self.logger.error("[ERROR] .env Token 测试失败")
                        return False
                except Exception as e:
                    self.logger.error(f"[ERROR] .env Token 连接失败: {e}")
                    return False

            # 两个都没有
            self.logger.error("[ERROR] Tushare token未配置，请在 Web 后台或 .env 文件中配置 TUSHARE_TOKEN")
            return False
