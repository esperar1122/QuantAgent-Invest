...
# Parse unified interface: start_dev.ps1 backend|frontend|worker [version]
if ($RemainingArgs.Count -gt 0) {
    ...
    switch ($cmd) {
        ...
        default {
            ...
            Write-Host "Usage: start_dev.ps1 [backend|frontend|worker] [version]" -ForegroundColor Yellow
            exit 1
        }
    }
}
...
if ($Help) {
    ...
    Write-Host "  .\start_dev.ps1                     # Start Backend API + Worker"
    Write-Host "  .\start_dev.ps1 backend             # Start Backend only"
    Write-Host "  .\start_dev.ps1 worker              # Start Worker only"
